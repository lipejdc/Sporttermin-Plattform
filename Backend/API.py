import service  # Imports a custom module likely containing business logic.
import json  # Used for handling JSON data.
import hashlib  # Used for hashing, such as passwords.
import datetime  # Used for handling date and time.
...
# Utility function to retrieve a user's ID from their session.
...
def login():
    ...
    if user != None and type(user) != list:
        ...
        session_id = hashlib.sha256(
            unencoded_session_id.encode("utf-8")).hexdigest()
...
def register():
    ...
    user = json.loads(service.user_create(
        first_name, last_name, email, passwd, age, city))
    if user != None:
        unencoded_session_id = str(user["id"]) + str(datetime.datetime.now())
        session_id = hashlib.sha256(
            unencoded_session_id.encode("utf-8")).hexdigest()
...
def change_profile():
    ...
    service.user_update(user_id, first_name, last_name,
                        user_info["email"], user_info["passwd"], age, city)

    return {}, 200

# Routes related to friendzone functionality, including creating and getting friendzones.
...
# Create a new friendzone


@app.route("/api/friendzoned/", methods=["POST"])
def fz_create():
    user_id = get_user_id_from_session(session)
    if user_id == "":
        return {}, 401
...
def apt_create():
    ...
    return service.apt_create(user_id=user_id, name=name, date=date, time_start=time_start, time_stop=time_stop, citycode=citycode, city=city, street=street, housenr=housenr, maxUser=maxUser, notice=notice, friendzone_id=friendzone_id), 200
...
# Routes for handling comments on appointments.
...
@app.route('/api/invite_fz/', methods=['POST'])
def invite():
    data = request.json
    friendzone_id = data.get('friendzone_id')
    user_email = data.get('user_email')

    if not friendzone_id or not user_email:
        return jsonify({'error': 'Fehlende Daten'}), 400

    result_message = service.create_and_send_invitation(
        friendzone_id, user_email)
    return jsonify({'message': result_message})

# Route to handle the acceptance of a friendzone invitation.
...
def accept_invitation():
    ...
    result_message = service.check_token_fz(token)
